    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#f5f5f5" }}>
      ...
      >
        ...
        >
          <TouchableOpacity
            onPress={() => router.back()}
            className="mx-6 mt-8 flex-row items-center"
          >
          ...
          <View className="mx-6 mt-8">
            ...
            >
              <Text
                className=" text-2xl font-interMedium mb-2"
                style={{ color: "#0A0A0A" }}
              >
                {ride?.rideName}
              </Text>
              <Text
                className="text-base mb-4 font-interRegular"
                style={{ color: "#4B5563" }}
              >
          ...
          <View className="flex-row mx-6 mt-8 mb-4 shadow-lg rounded-lg overflow-hidden">
            ...
            <TouchableOpacity
              ...
              className={`flex-1 py-2 ${activeTab === "invite" ? "bg-[#7B3FE4]" : "bg-gray-100"}`}
            >
              <Text
              ...
              <View>
                ...
                {isAdmin ? (
                  ride?.riders.length === 1 ? (
                    ...
                    <View className="bg-white rounded-xl p-6 items-center shadow-lg w-full max-w-sm mt-6">
                      {/* Navigate to Invite Button */}
                      <TouchableOpacity
                        onPress={() => setActiveTab("invite")}
                        style={{ backgroundColor: "#7B3FE4" }}
                        className="rounded-lg py-3 px-6 mb-4 w-full items-center"
                        activeOpacity={0.8}
                      >
                      ...
                      {/* Info Text */}
                      <Text className="text-center text-base font-interRegular leading-relaxed" style={{ color: "#4B5563" }}>
                        Invite members first to begin the ride!
                      </Text>
                    </View>
                  ) : (
                    // Admin + other riders
                  ...
                  <TouchableOpacity
                    style={{ backgroundColor: "#7B3FE4" }}
                    className="py-4 rounded-xl items-center justify-center mt-4 mb-10"
                    onPress={() => {
                      if (ride) handleRidePress(ride);
                    }}
                  >
                    <Text className="text-white font-interMedium">Join Ride</Text>
                  </TouchableOpacity>
                )}
              </View>
            ) : (
